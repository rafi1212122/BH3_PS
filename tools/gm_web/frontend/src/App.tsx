...
export default function App() {
    ...
    return <>
        <Header style={{ justifyContent: 'space-around', gap: '1vw' }} display={'flex'} p={'sm'} height={'auto'}>
            <Select w={'100%'} label="Session" value={selectedSession} onChange={setSelectedSession} data={sessions.map(session => ({ value: session.id, label: `UID ${session.uid} (${session.id})` }))}/>
